...
object ChatModel {
  ...
  // declaration of chatsContext should be before any other variable that is taken from ChatsContext
  // class and used in the model, otherwise, strange crash with NullPointerException for "this"
  // parameter in random functions
  ...
  // Used for deciding to add terminal items on main thread or not. Floating means
  // appPrefs.terminalAlwaysVisible
  ...
  val showChatPreviews by lazy {
    mutableStateOf(ChatController.appPrefs.privacyShowChatPreviews.get())
  }
  ...
  val migrationState: MutableState<MigrationToState?> by lazy {
    mutableStateOf(MigrationToDeviceState.makeMigrationState())
  }
  ...
  val networkInfo =
          mutableStateOf(UserNetworkInfo(networkType = UserNetworkType.OTHER, online = true))
  ...
  val desktopNoUserNoRemote: Boolean
    @Composable
    get() = appPlatform.isDesktop && currentUser.value == null && currentRemoteHost.value == null
  fun desktopNoUserNoRemote(): Boolean =
          appPlatform.isDesktop && currentUser.value == null && currentRemoteHost.value == null

  // remote controller
  val remoteHosts = mutableStateListOf<RemoteHostInfo>()
  val currentRemoteHost = mutableStateOf<RemoteHostInfo?>(null)
  val remoteHostId: Long?
    @Composable get() = remember { currentRemoteHost }.value?.remoteHostId
  ...
  fun chatsForContent(contentTag: MsgContentTag?): State<SnapshotStateList<Chat>> =
          when (contentTag) {
  ...
  fun chatItemsForContent(contentTag: MsgContentTag?): State<SnapshotStateList<ChatItem>> =
          when (contentTag) {
  ...
  fun chatStateForContent(contentTag: MsgContentTag?): ActiveChatState =
          when (contentTag) {
  ...
  fun chatItemsChangesListenerForContent(contentTag: MsgContentTag?): ChatItemsChangesListener? =
          when (contentTag) {
  ...
  fun setChatItemsChangeListenerForContent(
          listener: ChatItemsChangesListener?,
  ...
  private fun getUserIndex(user: User): Int =
          users.indexOfFirst {
            it.user.userId == user.userId && it.user.remoteHostId == user.remoteHostId
          }
  ...
  fun updateChatTags(rhId: Long?) {
    ...
    for (chat in chats.value.filter { it.remoteHostId == rhId }) {
      ...
      if (chat.unreadTag) {
        ...
        chatTags.forEach { tag -> newUnreadTags[tag] = (newUnreadTags[tag] ?: 0) + 1 }
      }
    }
  ...
  fun moveChatTagUnread(chat: Chat, oldTags: List<Long>?, newTags: List<Long>) {
    if (chat.unreadTag) {
      ...
      newTags.forEach { t -> unreadTags[t] = (unreadTags[t] ?: 0) + 1 }
    }
  }
  // TODO pass rhId?
  fun getChat(id: String): Chat? = chats.value.firstOrNull { it.id == id }
  fun getContactChat(contactId: Long): Chat? =
          chats.value.firstOrNull {
            it.chatInfo is ChatInfo.Direct && it.chatInfo.apiId == contactId
          }
  fun getGroupChat(groupId: Long): Chat? =
          chats.value.firstOrNull { it.chatInfo is ChatInfo.Group && it.chatInfo.apiId == groupId }

  fun populateGroupMembersIndexes() {
    groupMembersIndexes.value = emptyMap()
    val gmIndexes = groupMembersIndexes.value.toMutableMap()
    groupMembers.value.forEachIndexed { i, member -> gmIndexes[member.groupMemberId] = i }
  ...
  // running everything inside the block on main thread. Make sure any heavy computation is moved to
  // a background thread
  suspend fun <T> withChats(
          contentTag: MsgContentTag? = null,
          action: suspend ChatsContext.() -> T
  ): T =
  ...
  suspend fun <T> withReportsChatsIfOpen(action: suspend ChatsContext.() -> T) =
          withContext(Dispatchers.Main) {
  ...
  class ChatsContext(private val contentTag: MsgContentTag?) {
    ...
    /**
     * if you modify the items by adding/removing them, use helpers methods like [addAndNotify],
     * [removeLastAndNotify], [removeAllAndNotify], [clearAndNotify] and so on. If some helper is
     * missing, create it. Notify is needed to track state of items that we added manually (not via
     * api call). See [apiLoadMessages]. If you use api call to get the items, use just [add]
     * instead of [addAndNotify]. Never modify underlying list directly because it produces
     * unexpected results in ChatView's LazyColumn (setting by index is ok)
     */
    ...
    fun hasChat(rhId: Long?, id: String): Boolean =
            chats.value.firstOrNull { it.id == id && it.remoteHostId == rhId } != null
    fun getChat(id: String): Chat? = chats.value.firstOrNull { it.id == id }
    private fun getChatIndex(rhId: Long?, id: String): Int =
            chats.value.indexOfFirst { it.id == id && it.remoteHostId == rhId }
        ...
        if (currentCInfo is ChatInfo.Direct && newCInfo is ChatInfo.Direct) {
          ...
          if (currentStats != null && newConn != null && newStats == null) {
            newCInfo =
                    newCInfo.copy(
                            contact =
                                    newCInfo.contact.copy(
                                            activeConn =
                                                    newConn.copy(connectionStats = currentStats)
                                    )
                    )
          }
        }
    ...
    suspend fun updateContactConnection(rhId: Long?, contactConnection: PendingContactConnection) =
            updateChat(rhId, ChatInfo.ContactConnection(contactConnection))

    suspend fun updateContact(rhId: Long?, contact: Contact) =
            updateChat(rhId, ChatInfo.Direct(contact), addMissing = contact.directOrUsed)

    suspend fun updateContactConnectionStats(
            rhId: Long?,
            contact: Contact,
    ...
    suspend fun updateGroup(rhId: Long?, groupInfo: GroupInfo) =
            updateChat(rhId, ChatInfo.Group(groupInfo))
                ...
                chat.copy(
                        ...
                        chatStats =
                                if (cItem.meta.itemStatus is CIStatus.RcvNew) {
                                  ...
                                  chat.chatStats.copy(
                                          unreadCount = chat.chatStats.unreadCount + 1,
                                          unreadMentions =
                                                  if (cItem.meta.userMention)
                                                          chat.chatStats.unreadMentions + 1
                                                  else chat.chatStats.unreadMentions
                                  )
                                } else chat.chatStats
                )
        ...
        if (chatId.value == cInfo.id) {
          ...
          if (chatItems.value.none { it.id == cItem.id }) {
            if (chatItems.value.lastOrNull()?.id == ChatItem.TEMP_LIVE_CHAT_ITEM_ID) {
              chatItems.addAndNotify(
                      kotlin.math.max(0, chatItems.value.lastIndex),
                      cItem,
                      contentTag
              )
            } else {
      ...
      return withContext(Dispatchers.Main) {
        ...
        if (chatId.value == cInfo.id) {
          ...
          if (itemIndex >= 0) {
            ...
            val ci =
                    if (status != null && cItem.meta.itemStatus is CIStatus.SndNew) {
    ...
    fun removeChatItem(rhId: Long?, cInfo: ChatInfo, cItem: ChatItem) {
      ...
      if (chatId.value == cInfo.id) {
        chatItems.removeAllAndNotify {
          // We delete taking into account meta.createdAt to make sure we will not be in situation
          // when two items with the same id will be deleted
          // (it can happen if already deleted chat item in backend still in the list and new one
          // came with the same (re-used) chat item id)
    ...
    suspend fun removeMemberItems(
      ...
      fun removedUpdatedItem(item: ChatItem): ChatItem? {
        val newContent =
                when {
                  item.chatDir is CIDirection.GroupSnd &&
                          removedMember.groupMemberId == groupInfo.membership.groupMemberId ->
                          CIContent.SndModerated
                  item.chatDir is CIDirection.GroupRcv &&
                          item.chatDir.groupMember.groupMemberId == removedMember.groupMemberId ->
                        ...
                        meta =
                                item.meta.copy(
                                        itemDeleted =
                                                CIDeleted.Moderated(
                                                        Clock.System.now(),
                                                        byGroupMember = byMember
                                                )
                                ),
                        content =
                                if (groupInfo.fullGroupPreferences.fullDelete.on) newContent
                                else item.content
                )
      ...
      // this should not happen, only another member can "remove" user, user can only "leave"
      // (another event).
      ...
      init {
        withLongRunningApi {
          subject.throttleLatest(2000).collect {
            withChats(contentTag) { chats.replaceAll(popCollectedChats()) }
          }
        }
      }
    ...
    fun markChatItemsRead(remoteHostId: Long?, id: ChatId, itemIds: List<Long>? = null) {
      ...
      if (chatIdx >= 0) {
        ...
        if (lastId != null) {
          ...
          val unreadMentions =
                  if (itemIds != null) chat.chatStats.unreadMentions - mentionsMarkedRead else 0
          decreaseUnreadCounter(
                  remoteHostId,
                  currentUser.value!!,
                  chat.chatStats.unreadCount - unreadCount
          )
          chats[chatIdx] =
                  chat.copy(
                          chatStats =
    ...
    private fun markItemsReadInCurrentChat(
            id: ChatId,
      ...
      if (chatId.value == id) {
        ...
        while (i >= 0) {
          ...
          if (item.meta.itemStatus is CIStatus.RcvNew &&
                          (itemIds == null || itemIdsFromRange.contains(item.id))
          ) {
              ...
              items[i] =
                      newItem.copy(
                              meta =
                                      newItem.meta.copy(
                                              itemTimed =
                                                      newItem.meta.itemTimed.copy(
                                                              deleteAt =
                                                                      Clock.System.now() +
                                                                              newItem.meta.itemTimed
                                                                                      .ttl
                                                                                      .toDuration(
                                                                                              DurationUnit
                                                                                                      .SECONDS
    ...
    private fun decreaseCounterInChatNoContentTag(rhId: Long?, chatId: ChatId) {
      // updates anything only in main ChatView, not GroupReportsView or anything else from the
      // future
    ...
    suspend fun upsertGroupMember(rhId: Long?, groupInfo: GroupInfo, member: GroupMember): Boolean {
      ...
      return if (chatId.value == groupInfo.id) {
        if (groupMembers.value.isNotEmpty() &&
                        groupMembers.value.firstOrNull()?.groupId != groupInfo.groupId
        ) {
          // stale data, should be cleared at that point, otherwise, duplicated items will be here
          // which will produce crashes in LazyColumn
        ...
        val updated =
                chatItems.value.map {
                  // Take into account only specific changes, not all. Other member updates are not
                  // important and can be skipped
                  if (it.chatDir is CIDirection.GroupRcv &&
                                  it.chatDir.groupMember.groupMemberId == member.groupMemberId &&
                                  (it.chatDir.groupMember.image != member.image ||
                                          it.chatDir.groupMember.chatViewName !=
                                                  member.chatViewName ||
    ...
    suspend fun updateGroupMemberConnectionStats(
            rhId: Long?,
            groupInfo: GroupInfo,
            member: GroupMember,
            connectionStats: ConnectionStats
    ...
    fun updateChatTagReadNoContentTag(chat: Chat, wasUnread: Boolean) {
      ...
      if (nowUnread && !wasUnread) {
        tags.forEach { tag -> unreadTags[tag] = (unreadTags[tag] ?: 0) + 1 }
    ...
    fun markChatTagRead(chat: Chat) {
      if (chat.unreadTag) {
        chat.chatInfo.chatTags?.let { tags -> markChatTagReadNoContentTag_(chat, tags) }
      }
    }
        ...
        chats[i] =
                chat.copy(
                        chatStats =
                                chat.chatStats.copy(
                                        reportsCount =
                                                (chat.chatStats.reportsCount + by).coerceAtLeast(0),
                                )
                )
        val wasReportsCount = chat.chatStats.reportsCount
        val nowReportsCount = chats[i].chatStats.reportsCount
        val by =
                if (wasReportsCount == 0 && nowReportsCount > 0) 1
                else if (wasReportsCount > 0 && nowReportsCount == 0) -1 else 0
    ...
    private fun changeGroupReportsTagNoContentTag(by: Int = 0) {
      ...
      presetTags[PresetTagKind.GROUP_REPORTS] =
              kotlin.math.max(0, (presetTags[PresetTagKind.GROUP_REPORTS] ?: 0) + by)
      clearActiveChatFilterIfNeeded()
    }
  }
  ...
  fun updateCurrentUser(
          rhId: Long?,
          newProfile: Profile,
          preferences: FullChatPreferences? = null
  ) {
  ...
  suspend fun addLiveDummy(chatInfo: ChatInfo): ChatItem {
    ...
    withChats { chatItems.addAndNotify(cItem, contentTag = null) }
    return cItem
  }

  fun removeLiveDummy() {
    if (chatItemsForContent(null).value.lastOrNull()?.id == ChatItem.TEMP_LIVE_CHAT_ITEM_ID) {
      withApi { withChats { chatItems.removeLastAndNotify(contentTag = null) } }
    }
  }
  ...
  fun getPrevShownChatItem(
          ciIndex: Int?,
          ciCategory: CIMergeCategory?,
          reversedChatItems: List<ChatItem>
  ): Pair<Int?, ChatItem?> {
  ...
  fun getPrevHiddenMember(
          member: GroupMember,
          range: IntRange,
          reversedChatItems: List<ChatItem>
  ): Pair<GroupMember?, Int> {
  ...
  fun contactNetworkStatus(contact: Contact): NetworkStatus {
    val conn = contact.activeConn
    return if (conn != null) networkStatuses[conn.agentConnId] ?: NetworkStatus.Unknown()
  ...
  fun addTerminalItem(item: TerminalItem) {
    ...
    if (terminalsVisible.isNotEmpty()) {
      withApi { addTerminalItem(item, maxItems) }
    } else {
  ...
  val connectedToRemote: Boolean
    @Composable get() = currentRemoteHost.value != null || remoteCtrlSession.value?.active == true
  fun connectedToRemote(): Boolean =
          currentRemoteHost.value != null || remoteCtrlSession.value?.active == true
}
...
interface UserLike {
  ...
  val showNotifications: Boolean
    get() = activeUser || showNtfs
}

@Serializable data class UserPwdHash(val hash: String, val salt: String)

@Serializable
data class UserInfo(val user: User, val unreadCount: Int) {
  companion object {
...
interface NamedChat {
  ...
  val chatViewName: String
    get() =
            localAlias.ifEmpty {
              displayName + (if (fullName == "" || fullName == displayName) "" else " / $fullName")
            }

  fun anyNameContains(searchAnyCase: String): Boolean {
    val s = searchAnyCase.trim().lowercase()
    return chatViewName.lowercase().contains(s) ||
            displayName.lowercase().contains(s) ||
            fullName.lowercase().contains(s)
  }
}
...
data class Chat(
  ...
  @Transient var popTs: Instant? = null
  ...
  val unreadTag: Boolean
    get() =
            when (chatInfo.chatSettings?.enableNtfs) {
...
sealed class ChatInfo : SomeChat, NamedChat {
  ...
  data class Direct(val contact: Contact) : ChatInfo() {
    override val chatType
      get() = ChatType.Direct
    override val localDisplayName
      get() = contact.localDisplayName
    override val id
      get() = contact.id
    override val apiId
      get() = contact.apiId
    override val ready
      get() = contact.ready
    override val chatDeleted
      get() = contact.chatDeleted
    override val sendMsgEnabled
      get() = contact.sendMsgEnabled
    override val incognito
      get() = contact.incognito
    override fun featureEnabled(feature: ChatFeature) = contact.featureEnabled(feature)
    override val timedMessagesTTL: Int?
      get() = contact.timedMessagesTTL
    override val createdAt
      get() = contact.createdAt
    override val updatedAt
      get() = contact.updatedAt
    ...
    override val image
      get() = contact.image
    override val localAlias: String
      get() = contact.localAlias
    override fun anyNameContains(searchAnyCase: String): Boolean =
            contact.anyNameContains(searchAnyCase)
  ...
  data class Group(val groupInfo: GroupInfo) : ChatInfo() {
    override val chatType
      get() = ChatType.Group
    override val localDisplayName
      get() = groupInfo.localDisplayName
    override val id
      get() = groupInfo.id
    override val apiId
      get() = groupInfo.apiId
    override val ready
      get() = groupInfo.ready
    override val chatDeleted
      get() = groupInfo.chatDeleted
    override val sendMsgEnabled
      get() = groupInfo.sendMsgEnabled
    override val incognito
      get() = groupInfo.incognito
    override fun featureEnabled(feature: ChatFeature) = groupInfo.featureEnabled(feature)
    override val timedMessagesTTL: Int?
      get() = groupInfo.timedMessagesTTL
    override val createdAt
      get() = groupInfo.createdAt
    override val updatedAt
      get() = groupInfo.updatedAt
    override val displayName
      get() = groupInfo.displayName
    override val fullName
      get() = groupInfo.fullName
    override val image
      get() = groupInfo.image
    override val localAlias
      get() = groupInfo.localAlias
  ...
  data class Local(val noteFolder: NoteFolder) : ChatInfo() {
    override val chatType
      get() = ChatType.Local
    override val localDisplayName
      get() = noteFolder.localDisplayName
    override val id
      get() = noteFolder.id
    override val apiId
      get() = noteFolder.apiId
    override val ready
      get() = noteFolder.ready
    override val chatDeleted
      get() = noteFolder.chatDeleted
    override val sendMsgEnabled
      get() = noteFolder.sendMsgEnabled
    override val incognito
      get() = noteFolder.incognito
    override fun featureEnabled(feature: ChatFeature) = noteFolder.featureEnabled(feature)
    override val timedMessagesTTL: Int?
      get() = noteFolder.timedMessagesTTL
    override val createdAt
      get() = noteFolder.createdAt
    override val updatedAt
      get() = noteFolder.updatedAt
    override val displayName
      get() = noteFolder.displayName
    override val fullName
      get() = noteFolder.fullName
    override val image
      get() = noteFolder.image
    override val localAlias
      get() = noteFolder.localAlias
  ...
  class ContactRequest(val contactRequest: UserContactRequest) : ChatInfo() {
    override val chatType
      get() = ChatType.ContactRequest
    override val localDisplayName
      get() = contactRequest.localDisplayName
    override val id
      get() = contactRequest.id
    override val apiId
      get() = contactRequest.apiId
    override val ready
      get() = contactRequest.ready
    override val chatDeleted
      get() = contactRequest.chatDeleted
    override val sendMsgEnabled
      get() = contactRequest.sendMsgEnabled
    override val incognito
      get() = contactRequest.incognito
    override fun featureEnabled(feature: ChatFeature) = contactRequest.featureEnabled(feature)
    override val timedMessagesTTL: Int?
      get() = contactRequest.timedMessagesTTL
    override val createdAt
      get() = contactRequest.createdAt
    override val updatedAt
      get() = contactRequest.updatedAt
    override val displayName
      get() = contactRequest.displayName
    override val fullName
      get() = contactRequest.fullName
    override val image
      get() = contactRequest.image
    override val localAlias
      get() = contactRequest.localAlias
  ...
  class ContactConnection(val contactConnection: PendingContactConnection) : ChatInfo() {
    override val chatType
      get() = ChatType.ContactConnection
    override val localDisplayName
      get() = contactConnection.localDisplayName
    override val id
      get() = contactConnection.id
    override val apiId
      get() = contactConnection.apiId
    override val ready
      get() = contactConnection.ready
    override val chatDeleted
      get() = contactConnection.chatDeleted
    override val sendMsgEnabled
      get() = contactConnection.sendMsgEnabled
    override val incognito
      get() = contactConnection.incognito
    override fun featureEnabled(feature: ChatFeature) = contactConnection.featureEnabled(feature)
    override val timedMessagesTTL: Int?
      get() = contactConnection.timedMessagesTTL
    override val createdAt
      get() = contactConnection.createdAt
    override val updatedAt
      get() = contactConnection.updatedAt
    override val displayName
      get() = contactConnection.displayName
    override val fullName
      get() = contactConnection.fullName
    override val image
      get() = contactConnection.image
    override val localAlias
      get() = contactConnection.localAlias

    companion object {
      fun getSampleData(
              status: ConnStatus = ConnStatus.New,
              viaContactUri: Boolean = false
      ): ContactConnection =
  ...
  fun ntfsEnabled(ci: ChatItem): Boolean = ntfsEnabled(ci.meta.userMention)
  ...
  val nextNtfMode: MsgFilter?
    get() = this.chatSettings?.enableNtfs?.nextMode(mentions = this.hasMentions)
            ...
            when (this) {
              ...
              is Error ->
                      String.format(
                              generalGetString(
                                      MR.strings
                                              .trying_to_connect_to_server_to_receive_messages_with_error
                              ),
                              connectionError
                      )
...
data class Contact(
  ...
  override val apiId
    get() = contactId
  override val ready
    get() = activeConn?.connStatus == ConnStatus.Ready
  val sndReady
    get() = ready || activeConn?.connStatus == ConnStatus.SndReady
  val active
    get() = contactStatus == ContactStatus.Active
  override val sendMsgEnabled
    get() =
            (sndReady &&
                    active &&
                    !(activeConn?.connectionStats?.ratchetSyncSendProhibited ?: false) &&
                    !(activeConn?.connDisabled ?: true)) || nextSendGrpInv
  val nextSendGrpInv
    get() = contactGroupMemberId != null && !contactGrpInvSent
  override val incognito
    get() = contactConnIncognito
  override fun featureEnabled(feature: ChatFeature) =
          when (feature) {
  ...
  override val timedMessagesTTL: Int?
    get() =
            with(mergedPreferences.timedMessages) {
              if (enabled.forUser) userPreference.pref.ttl else null
            }
  override val displayName
    get() = localAlias.ifEmpty { profile.displayName }
  override val fullName
    get() = profile.fullName
  override val image
    get() = profile.image
  ...
  override val localAlias
    get() = profile.localAlias
  val verified
    get() = activeConn?.connectionCode != null

  override fun anyNameContains(searchAnyCase: String): Boolean {
    val s = searchAnyCase.trim().lowercase()
    return profile.chatViewName.lowercase().contains(s) ||
            profile.displayName.lowercase().contains(s) ||
            profile.fullName.lowercase().contains(s)
  }
  ...
  val contactConnIncognito = activeConn?.customUserProfileId != null

  fun allowsFeature(feature: ChatFeature): Boolean =
          when (feature) {
            ChatFeature.TimedMessages ->
                    mergedPreferences.timedMessages.contactPreference.allow != FeatureAllowed.NO
            ChatFeature.FullDelete ->
                    mergedPreferences.fullDelete.contactPreference.allow != FeatureAllowed.NO
            ChatFeature.Voice ->
                    mergedPreferences.voice.contactPreference.allow != FeatureAllowed.NO
            ChatFeature.Reactions ->
                    mergedPreferences.reactions.contactPreference.allow != FeatureAllowed.NO
            ChatFeature.Calls ->
                    mergedPreferences.calls.contactPreference.allow != FeatureAllowed.NO
          }

  fun userAllowsFeature(feature: ChatFeature): Boolean =
          when (feature) {
            ChatFeature.TimedMessages ->
                    mergedPreferences.timedMessages.userPreference.pref.allow != FeatureAllowed.NO
            ChatFeature.FullDelete ->
                    mergedPreferences.fullDelete.userPreference.pref.allow != FeatureAllowed.NO
            ChatFeature.Reactions ->
                    mergedPreferences.reactions.userPreference.pref.allow != FeatureAllowed.NO
            ChatFeature.Voice ->
                    mergedPreferences.voice.userPreference.pref.allow != FeatureAllowed.NO
            ChatFeature.Calls ->
                    mergedPreferences.calls.userPreference.pref.allow != FeatureAllowed.NO
          }
...
@Serializable data class NavigationInfo(val afterUnread: Int = 0, val afterTotal: Int = 0)
...
@Serializable class ContactSubStatus(val contact: Contact, val contactError: ChatError? = null)
...
@Serializable data class VersionRange(val minVersion: Int, val maxVersion: Int)

@Serializable data class SecurityCode(val securityCode: String, val verifiedAt: Instant)
...
data class Profile(
  ...
  val profileViewName: String
    get() {
      return if (fullName == "" || displayName == fullName) displayName
      else "$displayName ($fullName)"
    }

  fun toLocalProfile(profileId: Long): LocalProfile =
          LocalProfile(
                  profileId,
                  displayName,
                  fullName,
  ...
  companion object {
    val sampleData = Profile(displayName = "alice", fullName = "Alice")
  }
}
...
data class LocalProfile(
  ...
  fun toProfile(): Profile =
          Profile(displayName, fullName, image, localAlias, contactLink, preferences)
...
@Serializable class Group(val groupInfo: GroupInfo, var members: List<GroupMember>)

@Serializable
sealed class ForwardConfirmation {
  @Serializable
  @SerialName("filesNotAccepted")
  data class FilesNotAccepted(val fileIds: List<Long>) : ForwardConfirmation()
  @Serializable
  @SerialName("filesInProgress")
  data class FilesInProgress(val filesCount: Int) : ForwardConfirmation()
  @Serializable
  @SerialName("filesMissing")
  data class FilesMissing(val filesCount: Int) : ForwardConfirmation()
  @Serializable
  @SerialName("filesFailed")
  data class FilesFailed(val filesCount: Int) : ForwardConfirmation()
}
...
data class GroupInfo(
  ...
  override val apiId
    get() = groupId
  override val ready
    get() = membership.memberActive
  override val chatDeleted
    get() = false
  override val sendMsgEnabled
    get() = membership.memberActive
  override val incognito
    get() = membership.memberIncognito
  override fun featureEnabled(feature: ChatFeature) =
          when (feature) {
  ...
  override val timedMessagesTTL: Int?
    get() = with(fullGroupPreferences.timedMessages) { if (on) ttl else null }
  override val displayName
    get() = localAlias.ifEmpty { groupProfile.displayName }
  override val fullName
    get() = groupProfile.fullName
  override val image
    get() = groupProfile.image
...
@Serializable data class GroupRef(val groupId: Long, val localDisplayName: String)
...
data class GroupProfile(
  ...
  companion object {
    val sampleData = GroupProfile(displayName = "team", fullName = "My Team")
  }
}
...
data class GroupMember(
  ...
  override val fullName: String
    get() = memberProfile.fullName
  override val image: String?
    get() = memberProfile.image
  val contactLink: String? = memberProfile.contactLink
  val verified
    get() = activeConn?.connectionCode != null
  val blocked
    get() = blockedByAdmin || !memberSettings.showMessages
  ...
  override val chatViewName: String
    get() {
      val p = memberProfile
      val name =
              p.localAlias.ifEmpty {
                p.displayName +
                        (if (p.fullName == "" || p.fullName == p.displayName) ""
                        else " / ${p.fullName}")
              }
      return pastMember(name)
    }
  ...
  fun canBeRemoved(groupInfo: GroupInfo): Boolean {
    ...
    return memberStatus != GroupMemberStatus.MemRemoved &&
            memberStatus != GroupMemberStatus.MemLeft &&
  ...
  fun canChangeRoleTo(groupInfo: GroupInfo): List<GroupMemberRole>? =
          if (!canBeRemoved(groupInfo)) null
                  ...
                  groupInfo.membership.memberRole.let { userRole ->
...
@Serializable data class GroupMemberSettings(val showMessages: Boolean) {}

@Serializable data class GroupMemberRef(val groupMemberId: Long, val profile: Profile)

@Serializable data class GroupMemberIds(val groupMemberId: Long, val groupId: Long)
  ...
  val text: String
    get() =
            when (this) {
              ...
              MemPendingApproval ->
                      generalGetString(MR.strings.group_member_status_pending_approval)
  ...
  val shortText: String
    get() =
            when (this) {
              ...
              MemInvited -> generalGetString(MR.strings.group_member_status_invited)
              MemPendingApproval ->
                      generalGetString(MR.strings.group_member_status_pending_approval_short)
...
@Serializable
class LinkPreview(val uri: String, val title: String, val description: String, val image: String) {
  companion object {
    val sampleData =
            LinkPreview(
                    uri = "https://gelbooru.com",
                    title = "Gelbooru | Anime Art & Hentai Gallery - Free to Explore",
                    description =
                            "Browse millions of anime, manga, videos, and video game themed images on Gelbooru. Discover art with detailed tags. Contains explicit hentai content.",
                    image =
                            "data:image/jpeg;base64,"
            )
  }
}

@Serializable class MemberSubError(val member: GroupMemberIds, val memberError: ChatError)
...
class NoteFolder(
  ...
  override val timedMessagesTTL: Int?
    get() = null
  override val displayName
    get() = generalGetString(MR.strings.note_folder_local_display_name)
  override val fullName
    get() = ""
  override val image
    get() = null
...
class PendingContactConnection(
  ...
  override val sendMsgEnabled
    get() = false
  override val incognito
    get() = customUserProfileId != null
  override fun featureEnabled(feature: ChatFeature) = false
  override val timedMessagesTTL: Int?
    get() = null
  override val localDisplayName
    get() = String.format(generalGetString(MR.strings.connection_local_display_name), pccConnId)
  override val displayName: String
    get() {
  ...
  val initiated
    get() = (pccConnStatus.initiated ?: false) && !viaContactUri
      ...
      return if (initiated == null) ""
              ...
              generalGetString(
                      if (initiated && !viaContactUri)
                              if (incognito)
                                      MR.strings.description_you_shared_one_time_link_incognito
                              else MR.strings.description_you_shared_one_time_link
                      else if (viaContactUri)
                              if (groupLinkId != null)
                                      if (incognito) MR.strings.description_via_group_link_incognito
                                      else MR.strings.description_via_group_link
                              else if (incognito)
                                      MR.strings.description_via_contact_address_link_incognito
                              else MR.strings.description_via_contact_address_link
                      else if (incognito) MR.strings.description_via_one_time_link_incognito
                      else MR.strings.description_via_one_time_link
              )
    }
...
@Serializable
data class ChatItemDeletion(val deletedChatItem: AChatItem, val toChatItem: AChatItem? = null)

@Serializable class AChatItem(val chatInfo: ChatInfo, val chatItem: ChatItem)

@Serializable class ACIReaction(val chatInfo: ChatInfo, val chatReaction: CIReaction)

@Serializable data class MemberReaction(val groupMember: GroupMember, val reactionTs: Instant)
...
@Serializable
data class CIMention(val memberId: String, val memberRef: CIMentionMember?) {
  constructor(
...
data class ChatItem(
  ...
  val timestampText: String
    get() = meta.timestampText

  val text: String
    get() {
      val mc = content.msgContent
      return when {
        content.text == "" && file != null && mc is MsgContent.MCVoice ->
                String.format(
                        generalGetString(MR.strings.voice_message_with_duration),
                        durationText(mc.duration)
                )
  ...
  val isRcvNew: Boolean
    get() = meta.isRcvNew

  val allowAddReaction: Boolean
    get() = meta.itemDeleted == null && !isLiveDummy && (reactions.count { it.userReacted } < 3)

  val isLiveDummy: Boolean
    get() = meta.itemId == TEMP_LIVE_CHAT_ITEM_ID

  val encryptedFile: Boolean? =
          if (file?.fileSource == null) null else file.fileSource.cryptoArgs != null

  val memberDisplayName: String?
    get() =
            when (chatDir) {
              is CIDirection.GroupRcv ->
                      when (content) {
                        is CIContent.RcvGroupEventContent ->
                                when (val event = content.rcvGroupEvent) {
                                  is RcvGroupEvent.MemberProfileUpdated -> {
                                    val to = event.toProfile
                                    val from = event.fromProfile
                                    when {
                                      to.displayName != from.displayName ||
                                              to.fullName != from.fullName -> null
  ...
  val memberConnected: GroupMember?
    get() =
            when (chatDir) {
              is CIDirection.GroupRcv ->
                      when (content) {
            ...
            when (content) {
              ...
              is CIContent.RcvGroupEventContent ->
                      when (content.rcvGroupEvent) {
                        ...
                        is RcvGroupEvent.MemberCreatedContact -> null
                        else -> CIMergeCategory.RcvGroupEvent
                      }
              is CIContent.SndGroupEventContent ->
                      when (content.sndGroupEvent) {
                ...
                if (meta.itemDeleted == null) {
                  ...
                  if (chatDir.sent) CIMergeCategory.SndItemDeleted
                  else CIMergeCategory.RcvItemDeleted
                }
              }
            }
  ...
  fun memberToModerate(chatInfo: ChatInfo): Pair<GroupInfo, GroupMember?>? {
    return if (chatInfo is ChatInfo.Group && chatDir is CIDirection.GroupRcv) {
      ...
      if (m.memberRole >= GroupMemberRole.Moderator &&
                      m.memberRole >= chatDir.groupMember.memberRole &&
                      meta.itemDeleted == null
      ) {
  ...
  val isActiveReport: Boolean
    get() = isReport && !isDeletedContent && meta.itemDeleted == null

  val canBeDeletedForSelf: Boolean
    get() =
            (content.msgContent != null && !meta.isLive) ||
                    meta.itemDeleted != null ||
                    isDeletedContent ||
                    mergeCategory != null ||
                    showLocalDelete
  ...
  companion object {
            ...
            ChatItem(
                    ...
                    meta =
                            CIMeta.getSample(
                                    1,
                                    Clock.System.now(),
                                    "received invitation to join group team as admin",
                                    CIStatus.RcvRead()
                            ),
                    content =
                            CIContent.RcvGroupInvitation(
                                    groupInvitation = CIGroupInvitation.getSample(status = status),
    ...
    fun getGroupEventSample() =
            ChatItem(
                    ...
                    content =
                            CIContent.RcvGroupEventContent(
                                    rcvGroupEvent =
    ...
    fun liveDummy(direct: Boolean): ChatItem =
            ChatItem(
...
fun MutableState<SnapshotStateList<Chat>>.add(index: Int, elem: Chat) {
  value =
          SnapshotStateList<Chat>().apply {
            addAll(value)
            add(index, elem)
          }
}

fun MutableState<SnapshotStateList<ChatItem>>.addAndNotify(
        index: Int,
        elem: ChatItem,
        contentTag: MsgContentTag?
) {
  value =
          SnapshotStateList<ChatItem>().apply {
            addAll(value)
            add(index, elem)
            chatModel
                    .chatItemsChangesListenerForContent(contentTag)
                    ?.added(elem.id to elem.isRcvNew, index)
          }
}
...
fun <T> MutableList<T>.removeAll(predicate: (T) -> Boolean): Boolean =
        if (isEmpty()) false else remAll(predicate)
...
operator fun <T> State<SnapshotStateList<T>>.set(index: Int, elem: T) {
  value[index] = elem
}

val State<List<Any>>.size: Int
  get() = value.size
...
data class CIMeta(
  ...
  val timestampText: String
    get() = getTimestampText(itemTs, true)

  val recent: Boolean
    get() = updatedAt + 10.toDuration(DurationUnit.SECONDS) > Clock.System.now()
  val isLive: Boolean
    get() = itemLive == true
  val disappearing: Boolean
    get() = !isRcvNew && itemTimed?.deleteAt != null

  val isRcvNew: Boolean
    get() = itemStatus is CIStatus.RcvNew

  fun statusIcon(
          primaryColor: Color,
          metaColor: Color = CurrentColors.value.colors.secondary,
          paleMetaColor: Color = CurrentColors.value.colors.secondary
  ): Pair<ImageResource, Color>? = itemStatus.statusIcon(primaryColor, metaColor, paleMetaColor)
...
@Serializable data class CITimed(val ttl: Int, val deleteAt: Instant?)
  ...
  val dayMonthYear =
          time.format(
                  DateTimeFormatter.ofPattern(
                          if (Clock.System.now().toLocalDateTime(tz).year == time.year) "d MMM"
                          else "d MMM yyyy"
                  )
          )
...
sealed class CIStatus {
  ...
  @Serializable
  @SerialName("sndSent")
  class SndSent(val sndProgress: SndCIStatusProgress) : CIStatus()
  @Serializable
  @SerialName("sndRcvd")
  class SndRcvd(val msgRcptStatus: MsgReceiptStatus, val sndProgress: SndCIStatusProgress) :
          CIStatus()
  ...
  fun statusIcon(
          ...
          when (this) {
            ...
            is SndRcvd ->
                    when (this.msgRcptStatus) {
                      MsgReceiptStatus.Ok ->
                              when (this.sndProgress) {
                                SndCIStatusProgress.Complete ->
                                        MR.images.ic_double_check to metaColor
                                SndCIStatusProgress.Partial ->
                                        MR.images.ic_double_check to paleMetaColor
                              }
  ...
  val statusInto: Pair<String, String>?
    get() =
            when (this) {
              is SndNew -> null
              is SndSent -> null
              is SndRcvd -> null
              is SndErrorAuth ->
                      generalGetString(MR.strings.message_delivery_error_title) to
                              generalGetString(MR.strings.message_delivery_error_desc)
              is CISSndError ->
                      generalGetString(MR.strings.message_delivery_error_title) to
                              agentError.errorInfo
              is SndWarning ->
                      generalGetString(MR.strings.message_delivery_warning_title) to
                              agentError.errorInfo
              is RcvNew -> null
              is RcvRead -> null
...
sealed class SndError {
  ...
  @Serializable
  @SerialName("proxy")
  class Proxy(val proxyServer: String, val srvError: SrvError) : SndError()
  @Serializable
  @SerialName("proxyRelay")
  class ProxyRelay(val proxyServer: String, val srvError: SrvError) : SndError()
  ...
  val errorInfo: String
    get() =
            when (this) {
              ...
              is SndError.Relay ->
                      generalGetString(MR.strings.snd_error_relay).format(srvError.errorInfo)
              is SndError.Proxy ->
                      generalGetString(MR.strings.snd_error_proxy)
                              .format(proxyServer, srvError.errorInfo)
              is SndError.ProxyRelay ->
                      generalGetString(MR.strings.snd_error_proxy_relay)
                              .format(proxyServer, srvError.errorInfo)
              is SndError.Other ->
                      generalGetString(MR.strings.ci_status_other_error).format(sndError)
            }
}
...
sealed class GroupSndStatus {
  ...
  @Serializable
  @SerialName("rcvd")
  class Rcvd(val msgRcptStatus: MsgReceiptStatus) : GroupSndStatus()
  ...
  val statusInto: Pair<String, String>?
    get() =
            when (this) {
              is New -> null
              is Forwarded ->
                      generalGetString(MR.strings.message_forwarded_title) to
                              generalGetString(MR.strings.message_forwarded_desc)
              is Inactive ->
                      generalGetString(MR.strings.member_inactive_title) to
                              generalGetString(MR.strings.member_inactive_desc)
              is Sent -> null
              is Rcvd -> null
              is Error ->
                      generalGetString(MR.strings.message_delivery_error_title) to
...
sealed class CIDeleted {
  ...
  @Serializable
  @SerialName("blockedByAdmin")
  class BlockedByAdmin(val deletedTs: Instant?) : CIDeleted()
  @Serializable
  @SerialName("moderated")
  class Moderated(val deletedTs: Instant?, val byGroupMember: GroupMember) : CIDeleted()
}
...
sealed class CIForwardedFrom {
  ...
  class Contact(
          ...
          val msgDir: MsgDirection,
          val contactId: Long? = null,
          val chatItemId: Long? = null
  ) : CIForwardedFrom()
  @Serializable
  @SerialName("group")
  class Group(
  ...
  val chatTypeApiIdMsgId: Triple<ChatType, Long, Long?>?
    get() =
            when (this) {
              Unknown -> null
              is Contact ->
                      if (contactId != null) Triple(ChatType.Direct, contactId, chatItemId)
                      else null
...
sealed class CIContent : ItemContent {
  ...
  class SndDeleted(val deleteMode: CIDeleteMode) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  // legacy - since v4.3.0 itemDeleted field is used
  @Serializable
  ...
  @Serializable
  @SerialName("sndCall")
  class SndCall(val status: CICallStatus, val duration: Int) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvCall")
  class RcvCall(val status: CICallStatus, val duration: Int) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvIntegrityError")
  class RcvIntegrityError(val msgError: MsgErrorType) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvDecryptionError")
  class RcvDecryptionError(val msgDecryptError: MsgDecryptError, val msgCount: UInt) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvGroupInvitation")
  class RcvGroupInvitation(
          val groupInvitation: CIGroupInvitation,
          val memberRole: GroupMemberRole
  ) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  ...
  class RcvDirectEventContent(val rcvDirectEvent: RcvDirectEvent) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvGroupEvent")
  class RcvGroupEventContent(val rcvGroupEvent: RcvGroupEvent) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("sndGroupEvent")
  class SndGroupEventContent(val sndGroupEvent: SndGroupEvent) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvConnEvent")
  class RcvConnEventContent(val rcvConnEvent: RcvConnEvent) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("sndConnEvent")
  class SndConnEventContent(val sndConnEvent: SndConnEvent) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvChatFeature")
  class RcvChatFeature(
  ...
  class RcvGroupFeature(
          val groupFeature: GroupFeature,
          val preference: GroupPreference,
          val param: Int? = null,
          val memberRole_: GroupMemberRole?
  ) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  ...
  class RcvChatFeatureRejected(val feature: ChatFeature) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvGroupFeatureRejected")
  class RcvGroupFeatureRejected(val groupFeature: GroupFeature) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("sndModerated")
  object SndModerated : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvModerated")
  object RcvModerated : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  ...
  @Serializable
  @SerialName("sndDirectE2EEInfo")
  class SndDirectE2EEInfo(val e2eeInfo: E2EEInfo) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvDirectE2EEInfo")
  class RcvDirectE2EEInfo(val e2eeInfo: E2EEInfo) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("sndGroupE2EEInfo")
  class SndGroupE2EEInfo(val e2eeInfo: E2EEInfo) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("rcvGroupE2EEInfo")
  class RcvGroupE2EEInfo(val e2eeInfo: E2EEInfo) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  @Serializable
  @SerialName("invalidJSON")
  data class InvalidJSON(val json: String) : CIContent() {
    override val msgContent: MsgContent?
      get() = null
  }
  ...
  override val text: String
    get() =
            when (this) {
              ...
              is RcvGroupFeature ->
                      featureText(groupFeature, preference.enable.text, param, memberRole_)
              is SndGroupFeature ->
                      featureText(groupFeature, preference.enable.text, param, memberRole_)
              is RcvChatFeatureRejected ->
                      "${feature.text}: ${generalGetString(MR.strings.feature_received_prohibited)}"
              is RcvGroupFeatureRejected ->
                      "${groupFeature.text}: ${generalGetString(MR.strings.feature_received_prohibited)}"
  ...
  companion object {
    ...
    fun featureText(
            ...
            }) + (if (feature.hasRole && role != null) " (${roleText(role)})" else "")
    ...
    fun preferenceText(feature: Feature, allowed: FeatureAllowed, param: Int?): String =
            when {
              allowed != FeatureAllowed.NO && feature.hasParam && param != null ->
                      String.format(
                              generalGetString(MR.strings.feature_offered_item_with_param),
                              feature.text,
                              timeText(param)
                      )
              allowed != FeatureAllowed.NO ->
                      String.format(
                              generalGetString(MR.strings.feature_offered_item),
                              feature.text,
                              timeText(param)
                      )
              else ->
                      String.format(
                              generalGetString(MR.strings.feature_cancelled_item),
                              feature.text,
                              timeText(param)
                      )
            }
...
class CIQuote(
  ...
  override val text: String by lazy {
    if (content.text == "" && content is MsgContent.MCVoice) durationText(content.duration)
    else content.text
  }

  fun sender(membership: GroupMember?): String? =
          when (chatDir) {
            ...
            is CIDirection.GroupSnd -> membership?.displayName
                            ?: generalGetString(MR.strings.sender_you_pronoun)
  ...
  companion object {
    fun getSample(itemId: Long?, sentAt: Instant, text: String, chatDir: CIDirection?): CIQuote =
            CIQuote(
                    ...
                    itemId = itemId,
                    sentAt = sentAt,
                    content = MsgContent.MCText(text)
            )
  }
}
...
sealed class MsgReaction {
  @Serializable(with = MsgReactionSerializer::class)
  class Emoji(val emoji: MREmojiChar) : MsgReaction()
  @Serializable(with = MsgReactionSerializer::class)
  class Unknown(val type: String? = null, val json: JsonElement) : MsgReaction()
  ...
  companion object {
    val values: List<MsgReaction>
      get() = MREmojiChar.values().map(::Emoji)
    val old: List<MsgReaction>
      get() =
              listOf(
...
object MsgReactionSerializer : KSerializer<MsgReaction> {
  override val descriptor: SerialDescriptor =
          buildSerialDescriptor("MsgReaction", PolymorphicKind.SEALED) {
            element("Emoji", buildClassSerialDescriptor("Emoji") { element<String>("emoji") })
...
data class CIFile(
  ...
  val showStatusIconInSmallView: Boolean =
          when (fileStatus) {
  ...
  /**
   * DO NOT CALL this function in compose scope, [LaunchedEffect], [DisposableEffect] and so on.
   * Only with [withBGApi] or [runBlocking]. Otherwise, it will be canceled when moving to another
   * screen/item/view, etc
   */
  ...
  fun forwardingAllowed(): Boolean =
          when {
            chatModel.connectedToRemote() &&
                    cachedRemoteFileRequests[fileSource] != false &&
                    loaded -> true
  ...
  companion object {
    fun getSample(
            ...
            CIFile(
                    ...
                    fileName = fileName,
                    fileSize = fileSize,
                    fileSource = if (filePath == null) null else CryptoFile.plain(filePath),
                    fileStatus = fileStatus,
...
@Serializable
data class CryptoFile(val filePath: String, val cryptoArgs: CryptoFileArgs?) {

  val isAbsolutePath: Boolean
    get() = File(filePath).isAbsolute

  @Transient private var tmpFile: File? = null
...
@Serializable data class CryptoFileArgs(val fileKey: String, val fileNonce: String)

class CancelAction(val uiActionId: StringResource, val alert: AlertInfo)
...
private val sndCancelAction: CancelAction =
        CancelAction(
...
private val revokeCancelAction: CancelAction =
        CancelAction(
...
private val rcvCancelAction: CancelAction =
        CancelAction(
                uiActionId = MR.strings.stop_file__action,
                alert =
                        AlertInfo(
...
sealed class CIFileStatus {
  ...
  class SndTransfer(val sndProgress: Long, val sndTotal: Long) : CIFileStatus()
  ...
  @Serializable
  @SerialName("sndError")
  class SndError(val sndFileError: FileError) : CIFileStatus()
  @Serializable
  @SerialName("sndWarning")
  class SndWarning(val sndFileError: FileError) : CIFileStatus()
  ...
  class RcvTransfer(val rcvProgress: Long, val rcvTotal: Long) : CIFileStatus()
  ...
  @Serializable
  @SerialName("rcvError")
  class RcvError(val rcvFileError: FileError) : CIFileStatus()
  @Serializable
  @SerialName("rcvWarning")
  class RcvWarning(val rcvFileError: FileError) : CIFileStatus()
...
sealed class FileError {
  ...
  @Serializable
  @SerialName("blocked")
  class Blocked(val server: String, val blockInfo: BlockingInfo) : FileError()
  ...
  val errorInfo: String
    get() =
            when (this) {
              ...
              is Blocked ->
                      generalGetString(MR.strings.file_error_blocked).format(blockInfo.reason.text)
  ...
  val moreInfoButton: Pair<String, String>?
    get() =
            when (this) {
...
sealed class MsgContent {
  ...
  @Serializable(with = MsgContentSerializer::class)
  class MCVideo(override val text: String, val image: String, val duration: Int) : MsgContent()
  @Serializable(with = MsgContentSerializer::class)
  class MCVoice(override val text: String, val duration: Int) : MsgContent()
  @Serializable(with = MsgContentSerializer::class)
  class MCFile(override val text: String) : MsgContent()
  @Serializable(with = MsgContentSerializer::class)
  class MCReport(override val text: String, val reason: ReportReason) : MsgContent()
  @Serializable(with = MsgContentSerializer::class)
  class MCUnknown(val type: String? = null, override val text: String, val json: JsonElement) :
          MsgContent()
  ...
  val isMediaOrFileAttachment: Boolean
    get() =
            when (this) {
              is MCImage -> true
              is MCVideo -> true
  ...
  val cmdString: String
    get() = if (this is MCUnknown) "json $json" else "json ${json.encodeToString(this)}"
}
...
@Serializable class E2EEInfo(val pqEnabled: Boolean) {}

object MsgContentSerializer : KSerializer<MsgContent> {
  override val descriptor: SerialDescriptor =
          buildSerialDescriptor("MsgContent", PolymorphicKind.SEALED) {
            element("MCText", buildClassSerialDescriptor("MCText") { element<String>("text") })
            element(
                    "MCLink",
                    buildClassSerialDescriptor("MCLink") {
                      element<String>("text")
            ...
            element(
                    "MCImage",
                    buildClassSerialDescriptor("MCImage") {
                      element<String>("text")
            ...
            element(
                    "MCVideo",
                    buildClassSerialDescriptor("MCVideo") {
                      element<String>("text")
                      element<String>("image")
                      element<Int>("duration")
                    }
            )
            element("MCFile", buildClassSerialDescriptor("MCFile") { element<String>("text") })
            element(
                    "MCReport",
                    buildClassSerialDescriptor("MCReport") {
                      element<String>("text")
  ...
  override fun deserialize(decoder: Decoder): MsgContent {
    ...
    return if (json is JsonObject) {
      if ("type" in json) {
        ...
        MsgContent.MCUnknown(
                text = generalGetString(MR.strings.invalid_message_format),
                json = json
        )
      }
    } else {
...
class FormattedText(val text: String, val format: Format? = null) {
  ...
  fun link(mode: SimplexLinkMode): String? =
          when (format) {
            is Format.Uri -> text
            is Format.SimplexLink ->
                    if (mode == SimplexLinkMode.BROWSER) text else format.simplexUri
  ...
  fun viewText(mode: SimplexLinkMode): String =
          if (format is Format.SimplexLink && mode == SimplexLinkMode.DESCRIPTION)
                  simplexLinkText(format.linkType, format.smpHosts)
          else text
  ...
  companion object {
    fun plain(text: String): List<FormattedText> =
            if (text.isEmpty()) emptyList() else listOf(FormattedText(text))
  }
}
...
sealed class Format {
  ...
  @Serializable
  @SerialName("simplexLink")
  class SimplexLink(
          val linkType: SimplexLinkType,
          val simplexUri: String,
          val smpHosts: List<String>
  ) : Format()
...
@Serializable class SndFileTransfer() {}
...
sealed class MsgErrorType() {
  @Serializable
  @SerialName("msgSkipped")
  class MsgSkipped(val fromMsgId: Long, val toMsgId: Long) : MsgErrorType()
  ...
  val text: String
    get() =
            when (this) {
              is MsgSkipped ->
                      String.format(
                              generalGetString(MR.strings.integrity_msg_skipped),
                              toMsgId - fromMsgId + 1
                      )
              ...
              is MsgDuplicate ->
                      generalGetString(MR.strings.integrity_msg_duplicate) // not used now
            }
}
...
sealed class RcvDirectEvent() {
  ...
  class ProfileUpdated(val fromProfile: Profile, val toProfile: Profile) : RcvDirectEvent()
  ...
  private fun profileUpdatedText(from: Profile, to: Profile): String =
          when {
            to.displayName != from.displayName || to.fullName != from.fullName ->
                    generalGetString(MR.strings.profile_update_event_contact_name_changed)
                            .format(from.profileViewName, to.profileViewName)
            to.image != from.image ->
                    when (to.image) {
            ...
            // shouldn't happen if backend correctly creates item; UI should be synchronized with
            // backend
...
sealed class RcvGroupEvent() {
  ...
  class MemberAdded(val groupMemberId: Long, val profile: Profile) : RcvGroupEvent()
  ...
  class MemberRole(val groupMemberId: Long, val profile: Profile, val role: GroupMemberRole) :
          RcvGroupEvent()
  @Serializable
  @SerialName("memberBlocked")
  class MemberBlocked(val groupMemberId: Long, val profile: Profile, val blocked: Boolean) :
          RcvGroupEvent()
  @Serializable @SerialName("userRole") class UserRole(val role: GroupMemberRole) : RcvGroupEvent()
  @Serializable
  @SerialName("memberDeleted")
  class MemberDeleted(val groupMemberId: Long, val profile: Profile) : RcvGroupEvent()
  ...
  class GroupUpdated(val groupProfile: GroupProfile) : RcvGroupEvent()
  ...
  @Serializable
  @SerialName("memberProfileUpdated")
  class MemberProfileUpdated(val fromProfile: Profile, val toProfile: Profile) : RcvGroupEvent()
  ...
  val text: String
    get() =
            when (this) {
              is MemberAdded ->
                      String.format(
                              generalGetString(MR.strings.rcv_group_event_member_added),
                              profile.profileViewName
                      )
              ...
              is MemberRole ->
                      String.format(
                              generalGetString(MR.strings.rcv_group_event_changed_member_role),
                              profile.profileViewName,
                              role.text
                      )
              is MemberBlocked ->
                      if (blocked) {
                        String.format(
                                generalGetString(MR.strings.rcv_group_event_member_blocked),
                                profile.profileViewName
                        )
                      } else {
                        String.format(
                                generalGetString(MR.strings.rcv_group_event_member_unblocked),
                                profile.profileViewName
                        )
                      }
              is UserRole ->
                      String.format(
                              generalGetString(MR.strings.rcv_group_event_changed_your_role),
                              role.text
                      )
              is MemberDeleted ->
                      String.format(
                              generalGetString(MR.strings.rcv_group_event_member_deleted),
                              profile.profileViewName
                      )
              ...
              is InvitedViaGroupLink ->
                      generalGetString(MR.strings.rcv_group_event_invited_via_your_group_link)
              is MemberCreatedContact ->
                      generalGetString(MR.strings.rcv_group_event_member_created_contact)
  ...
  private fun profileUpdatedText(from: Profile, to: Profile): String =
          when {
            to.displayName != from.displayName || to.fullName != from.fullName ->
                    generalGetString(MR.strings.profile_update_event_member_name_changed)
                            .format(from.profileViewName, to.profileViewName)
            to.image != from.image ->
                    when (to.image) {
                      null -> generalGetString(MR.strings.profile_update_event_removed_picture)
...
sealed class SndGroupEvent() {
  ...
  @Serializable
  @SerialName("memberDeleted")
  class MemberDeleted(val groupMemberId: Long, val profile: Profile) : SndGroupEvent()
  @Serializable @SerialName("userLeft") class UserLeft() : SndGroupEvent()
  @Serializable
  @SerialName("groupUpdated")
  class GroupUpdated(val groupProfile: GroupProfile) : SndGroupEvent()
  ...
  val text: String
    get() =
            when (this) {
              is MemberRole ->
                      String.format(
                              generalGetString(MR.strings.snd_group_event_changed_member_role),
                              profile.profileViewName,
                              role.text
                      )
              is UserRole ->
                      String.format(
                              generalGetString(
                                      MR.strings.snd_group_event_changed_role_for_yourself
                              ),
                              role.text
                      )
              is MemberBlocked ->
                      if (blocked) {
                        String.format(
                                generalGetString(MR.strings.snd_group_event_member_blocked),
                                profile.profileViewName
                        )
                      } else {
                        String.format(
                                generalGetString(MR.strings.snd_group_event_member_unblocked),
                                profile.profileViewName
                        )
                      }
              is MemberDeleted ->
                      String.format(
                              generalGetString(MR.strings.snd_group_event_member_deleted),
                              profile.profileViewName
                      )
...
sealed class RcvConnEvent {
  @Serializable
  @SerialName("switchQueue")
  class SwitchQueue(val phase: SwitchPhase) : RcvConnEvent()
  @Serializable
  @SerialName("ratchetSync")
  class RatchetSync(val syncStatus: RatchetSyncState) : RcvConnEvent()
  ...
  val text: String
    get() =
            when (this) {
              ...
              is VerificationCodeReset ->
                      generalGetString(MR.strings.rcv_conn_event_verification_code_reset)
              is PQEnabled ->
                      if (enabled) {
...
sealed class SndConnEvent {
  @Serializable
  @SerialName("switchQueue")
  class SwitchQueue(val phase: SwitchPhase, val member: GroupMemberRef? = null) : SndConnEvent()
  @Serializable
  @SerialName("ratchetSync")
  class RatchetSync(val syncStatus: RatchetSyncState, val member: GroupMemberRef? = null) :
          SndConnEvent()
    ...
    get() =
            when (this) {
              is SwitchQueue -> {
                  ...
                  return when (phase) {
                    SwitchPhase.Completed ->
                            String.format(
                                    generalGetString(
                                            MR.strings
                                                    .snd_conn_event_switch_queue_phase_completed_for_member
                                    ),
                                    it
                            )
                    else ->
                            String.format(
                                    generalGetString(
                                            MR.strings
                                                    .snd_conn_event_switch_queue_phase_changing_for_member
                                    ),
                                    it
                            )
                  }
                ...
                when (phase) {
                  SwitchPhase.Completed ->
                          generalGetString(MR.strings.snd_conn_event_switch_queue_phase_completed)
              ...
              is RatchetSync -> {
                member?.profile?.profileViewName?.let {
                  return when (syncStatus) {
                    RatchetSyncState.Ok ->
                            String.format(
                                    generalGetString(MR.strings.snd_conn_event_ratchet_sync_ok),
                                    it
                            )
                    RatchetSyncState.Allowed ->
                            String.format(
                                    generalGetString(
                                            MR.strings.snd_conn_event_ratchet_sync_allowed
                                    ),
                                    it
                            )
                    RatchetSyncState.Required ->
                            String.format(
                                    generalGetString(
                                            MR.strings.snd_conn_event_ratchet_sync_required
                                    ),
                                    it
                            )
                    RatchetSyncState.Started ->
                            String.format(
                                    generalGetString(
                                            MR.strings.snd_conn_event_ratchet_sync_started
                                    ),
                                    it
                            )
                    RatchetSyncState.Agreed ->
                            String.format(
                                    generalGetString(MR.strings.snd_conn_event_ratchet_sync_agreed),
                                    it
                            )
                  }
                }
...
sealed class ChatItemTTL : Comparable<ChatItemTTL> {
  ...
  override fun compareTo(other: ChatItemTTL): Int =
          (seconds.takeIf { it != 0L } ?: Long.MAX_VALUE).compareTo(
                  other.seconds.takeIf { it != 0L } ?: Long.MAX_VALUE
          )
...
@Serializable
data class ChatTag(val chatTagId: Long, val chatTagText: String, val chatTagEmoji: String?)
...
enum class NotificationPreviewMode {
  ...
  companion object {
    val default: NotificationPreviewMode = CONTACT
  }
}
...
sealed class RemoteCtrlSessionState {
  ...
  @Serializable
  @SerialName("pendingConfirmation")
  data class PendingConfirmation(val sessionCode: String) : RemoteCtrlSessionState()
  @Serializable
  @SerialName("connected")
  data class Connected(val sessionCode: String) : RemoteCtrlSessionState()
}

@Serializable
sealed class RemoteCtrlStopReason {
  @Serializable
  @SerialName("discoveryFailed")
  class DiscoveryFailed(val chatError: ChatError) : RemoteCtrlStopReason()
  @Serializable
  @SerialName("connectionFailed")
  class ConnectionFailed(val chatError: ChatError) : RemoteCtrlStopReason()
  @Serializable
  @SerialName("setupFailed")
  class SetupFailed(val chatError: ChatError) : RemoteCtrlStopReason()
...
sealed class UIRemoteCtrlSessionState {
  ...
  data class Found(val remoteCtrl: RemoteCtrlInfo, val compatible: Boolean) :
          UIRemoteCtrlSessionState()
  data class Connecting(val remoteCtrl_: RemoteCtrlInfo? = null) : UIRemoteCtrlSessionState()
  data class PendingConfirmation(val remoteCtrl_: RemoteCtrlInfo? = null, val sessionCode: String) :
          UIRemoteCtrlSessionState()
  data class Connected(val remoteCtrl: RemoteCtrlInfo, val sessionCode: String) :
          UIRemoteCtrlSessionState()
}
