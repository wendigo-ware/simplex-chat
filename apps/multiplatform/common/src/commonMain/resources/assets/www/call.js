...
function localMedia(call) {
  return call.localMediaSources.camera || call.localMediaSources.screenVideo
    ? CallMediaType.Video
    : CallMediaType.Audio;
}
function peerMedia(call) {
  return call.peerMediaSources.camera || call.peerMediaSources.screenVideo
    ? CallMediaType.Video
    : CallMediaType.Audio;
}
...
const processCommand = (function () {
  const defaultIceServers = [
    { urls: ["stun:adminforge.de:3478"] },
    { urls: ["stun:disroot.org:3478"] },
    { urls: ["stun:turn.envs.net:3478"] },
    { urls: ["stun:danwin1210.de:3478"] },
    { urls: ["stun:matrix.grin.hu:3478"] },
  ];
  function getCallConfig(encodedInsertableStreams, iceServers, relay) {
    return {
      peerConnectionConfig: {
        iceServers:
          iceServers !== null && iceServers !== void 0
            ? iceServers
            : defaultIceServers,
  ...
  async function initializeCall(config, mediaType, aesKey) {
    ...
    try {
      ...
      console.log(
        "Error while constructing RTCPeerConnection, will try without 'stuns' specified: " +
          e,
      );
      const withoutStuns =
        (_a = config.peerConnectionConfig.iceServers) === null || _a === void 0
          ? void 0
          : _a.filter((elem) =>
              typeof elem.urls === "string"
                ? !elem.urls.startsWith("stuns:")
                : !elem.urls.some((url) => url.startsWith("stuns:")),
            );
    ...
    const localCamera =
      (_b =
        notConnectedCall === null || notConnectedCall === void 0
          ? void 0
          : notConnectedCall.localCamera) !== null && _b !== void 0
        ? _b
        : VideoCamera.User;
    let localStream;
    try {
    ...
    const call = {
      ...
      layout:
        (_c =
          notConnectedCall === null || notConnectedCall === void 0
            ? void 0
            : notConnectedCall.layout) !== null && _c !== void 0
          ? _c
          : LayoutType.Default,
    ...
    async function connectionHandler() {
      ...
      sendMessageToNative({
        resp: {
          ...
          state: {
            connectionState:
              (_a = pc.connectionState) !== null && _a !== void 0
                ? _a
  ...
  async function processCommand(body) {
    ...
    const pc =
      activeCall === null || activeCall === void 0
        ? void 0
    ...
    try {
      switch (command.type) {
        case "capabilities":
          ...
          try {
            localStream = await getLocalMediaStream(
              true,
              command.media == CallMediaType.Video &&
                (await browserHasCamera()),
              VideoCamera.User,
            );
            ...
            resp = {
              type: "error",
              message:
                "capabilities: no permissions were granted for mic and/or camera",
            };
            break;
            localStream = new MediaStream();
            // Will be shown on the next stage of call establishing, can work without any streams
            //desktopShowPermissionsAlert(command.media)
          }
          // Specify defaults that can be changed via UI before call established. It's only used before activeCall instance appears
          inactiveCallMediaSources.mic =
            localStream != null && localStream.getAudioTracks().length > 0;
          inactiveCallMediaSources.camera =
            localStream != null && localStream.getVideoTracks().length > 0;
        ...
        case "start": {
          ...
          activeCall = await initializeCall(
            getCallConfig(encryption && !!aesKey, iceServers, relay),
            media,
            aesKey,
          );
        ...
        case "offer":
          if (activeCall) {
            ...
            resp = {
              type: "error",
              message: "accept: encryption is not supported",
            };
          } else {
            ...
            activeCall = await initializeCall(
              getCallConfig(!!aesKey, iceServers, relay),
              media,
              aesKey,
            );
            const pc = activeCall.connection;
            ...
            await pc.setRemoteDescription(
              new RTCSessionDescription(
                !webView69Or70() ? offer : adaptSdpToOldWebView(offer),
              ),
            );
        ...
        case "answer":
          if (!pc) {
            ...
            resp = {
              type: "error",
              message: "answer: local description is not set",
            };
          } else if (pc.currentRemoteDescription) {
            resp = {
              type: "error",
              message: "answer: remote description already set",
            };
          } else {
            ...
            await pc.setRemoteDescription(
              new RTCSessionDescription(
                !webView69Or70() ? answer : adaptSdpToOldWebView(answer),
              ),
            );
            adaptToOldVersion(
              pc.getTransceivers()[2].currentDirection == "sendonly",
              activeCall,
            );
            addIceCandidates(pc, remoteIceCandidates);
            addIceCandidates(pc, afterCallInitializedCandidates);
        ...
        case "ice":
          ...
          if (pc) {
            ...
            resp = {
              type: "error",
              message: "ice: call not started yet, will add candidates later",
            };
          }
          break;
        case "media":
          if (!activeCall) {
            if (!notConnectedCall) {
              ...
              resp = {
                type: "error",
                message: "media: call has not yet pass capabilities stage",
              };
              break;
            }
            ...
            recreateLocalStreamWhileNotConnected(
              (_a =
                notConnectedCall === null || notConnectedCall === void 0
                  ? void 0
                  : notConnectedCall.localCamera) !== null && _a !== void 0
                ? _a
                : VideoCamera.User,
            );
            resp = { type: "ok" };
          } else if (
            activeCall.peerHasOldVersion &&
            command.source == CallMediaSource.Camera &&
            !activeCall.peerMediaSources.camera
          ) {
            resp = {
              type: "error",
              message:
                "media: cannot enable camera since the peer has an old version",
            };
          } else if (
            !activeCall.cameraTrackWasSetBefore &&
            command.source == CallMediaSource.Camera &&
            command.enable
          ) {
            ...
            (command.source == CallMediaSource.Mic &&
              activeCall.localStream.getAudioTracks().length > 0) ||
            (command.source == CallMediaSource.Camera &&
              activeCall.localStream.getVideoTracks().length > 0)
          ) {
            if (
              enableMedia(
              ...
              resp = {
                type: "error",
                message: "media: cannot enable media source",
              };
            }
          } else {
            if (
              ...
              resp = {
                type: "error",
                message: "media: cannot replace media source",
              };
            }
          }
          break;
        case "camera":
          if (!activeCall || !pc) {
            if (notConnectedCall) {
              ...
              resp = {
                type: "error",
                message: "camera: call has not yet pass capabilities stage",
              };
            }
          } else {
            if (
              ...
              resp = {
                type: "error",
                message: "camera: cannot replace media source",
              };
            }
          }
          break;
  ...
  function endCall() {
    ...
    try {
      (_a =
        activeCall === null || activeCall === void 0
          ? void 0
          : activeCall.connection) === null || _a === void 0
        ? void 0
        : _a.close();
    } catch (e) {
      console.log(e);
  ...
  async function setupEncryptionWorker(call) {
    if (call.aesKey) {
      ...
      if (useWorker && !call.worker) {
        ...
        call.worker = new Worker(
          URL.createObjectURL(
            new Blob([workerCode], { type: "text/javascript" }),
          ),
        );
        call.worker.onerror = ({ error, filename, lineno, message }) =>
          console.log({ error, filename, lineno, message });
  ...
  async function setupLocalStream(incomingCall, call) {
    ...
    if (incomingCall) {
      ...
      if (transceivers.length > 2) {
        ...
        await ((_b =
          (_a = transceivers.find(
            (elem) =>
              mediaSourceFromTransceiverMid(elem.mid) == CallMediaSource.Mic,
          )) === null || _a === void 0
            ? void 0
            : _a.sender) === null || _b === void 0
          ? void 0
          : _b.replaceTrack(audioTracks[0]));
        await ((_d =
          (_c = transceivers.find(
            (elem) =>
              mediaSourceFromTransceiverMid(elem.mid) == CallMediaSource.Camera,
          )) === null || _c === void 0
            ? void 0
            : _c.sender) === null || _d === void 0
          ? void 0
          : _d.replaceTrack(videoTracks[0]));
      } else {
    ...
    if (call.aesKey && call.key) {
      ...
      for (const transceiver of pc.getTransceivers()) {
        ...
        setupPeerTransform(
          ...
          call.aesKey,
          call.key,
          source == CallMediaSource.Camera ||
            source == CallMediaSource.ScreenVideo
            ? CallMediaType.Video
            : CallMediaType.Audio,
        );
        mid++;
      ...
      try {
        if (call.aesKey && call.key) {
          ...
          setupPeerTransform(
            ...
            call.aesKey,
            call.key,
            event.receiver.track.kind == "video"
              ? CallMediaType.Video
              : CallMediaType.Audio,
          );
        }
        setupMuteUnmuteListener(event.transceiver, track);
        const mediaSource = mediaSourceFromTransceiverMid(
      ...
      for (const t of call.connection.getTransceivers()) {
        ...
        if (
          ...
          try {
            ...
            console.log(
              "Failed to set codec preferences, trying without any preferences: " +
                error,
            );
          }
        }
  ...
  async function startSendingCamera(call, camera) {
    ...
    const tc = pc
      .getTransceivers()
      .find(
        (tc) => tc.receiver.track.kind == "video" && tc.direction == "sendrecv",
      );
    if (!tc) {
    ...
    console.log(
      pc.getTransceivers().map((elem) => {
        var _a, _b;
        return (
          "" +
          ((_a = elem.sender.track) === null || _a === void 0
            ? void 0
            : _a.kind) +
          " " +
          ((_b = elem.receiver.track) === null || _b === void 0
            ? void 0
            : _b.kind) +
          " " +
          elem.direction
  ...
  toggleScreenShare = async function () {
    ...
    if (!call.localMediaSources.screenVideo) {
      ...
      pc.getTransceivers().forEach((elem) => {
        ...
        const screenAudioTrack = call.localScreenStream
          .getTracks()
          .find((elem) => elem.kind == "audio");
        const screenVideoTrack = call.localScreenStream
          .getTracks()
          .find((elem) => elem.kind == "video");
      ...
      pc.getTransceivers().forEach((elem) => {
        const source = mediaSourceFromTransceiverMid(elem.mid);
        if (
          source == CallMediaSource.ScreenAudio ||
          source == CallMediaSource.ScreenVideo
        ) {
  ...
  async function replaceMedia(call, source, enable, camera) {
    ...
    for (const t of source == CallMediaSource.Mic
    ...
    try {
      ...
      desktopShowPermissionsAlert(
        source == CallMediaSource.Mic
          ? CallMediaType.Audio
          : CallMediaType.Video,
      );
      return false;
    ...
    videos.local
      .play()
      .catch((e) =>
        console.log("replace media: local play", JSON.stringify(e)),
      );
    call.localMediaSources.mic = call.localStream.getAudioTracks().length > 0;
    call.localMediaSources.camera =
  ...
  function replaceTracks(pc, source, tracks) {
    ...
    const sender =
      (_a = pc
        .getTransceivers()
        .find((elem) => mediaSourceFromTransceiverMid(elem.mid) == source)) ===
        null || _a === void 0
        ? void 0
        : _a.sender;
    if (sender) {
  ...
  async function recreateLocalStreamWhileNotConnected(newCamera) {
    ...
    const oldCamera =
      notConnectedCall === null || notConnectedCall === void 0
        ? void 0
        : notConnectedCall.localCamera;
    if (!localStream || !oldCamera || !videos) return;
    if (!inactiveCallMediaSources.mic) {
      localStream
        .getAudioTracks()
        .forEach((elem) =>
          isDesktop || stopTrackOnAndroid
            ? elem.stop()
            : (elem.enabled = false),
        );
      localStream
        .getAudioTracks()
        .forEach((elem) => localStream.removeTrack(elem));
    }
    ...
    await getLocalMediaStream(
      ...
      inactiveCallMediaSources.camera &&
        (localStream.getVideoTracks().length == 0 || oldCamera != newCamera) &&
        (await browserHasCamera()),
      newCamera,
    )
      .then((stream) => {
  ...
  function setupPeerTransform(operation, peer, worker, aesKey, key, media) {
    if (worker && "RTCRtpScriptTransform" in window) {
      ...
      peer.transform = new RTCRtpScriptTransform(worker, {
        operation,
        aesKey,
        media,
      });
      ...
      if (worker) {
        ...
        worker.postMessage({ operation, readable, writable, aesKey, media }, [
          readable,
          writable,
        ]);
      } else {
  ...
  function setupMuteUnmuteListener(transceiver, track) {
    ...
    let statsInterval = setInterval(async () => {
      ...
      if (inboundStatsId) {
        ...
        const bytes =
          (_a = stats.get(inboundStatsId)) === null || _a === void 0
            ? void 0
            : _a.bytesReceived;
  ...
  function onMediaMuteUnmute(transceiverMid, mute) {
    ...
    console.log(
      `Mute/unmute ${source} track = ${mute} with mid = ${transceiverMid}`,
    );
  ...
  function addRemoteTracksWhenUnmuted(source, call) {
    ...
    if (track) {
      const stream =
        source == CallMediaSource.Mic || source == CallMediaSource.Camera
          ? call.remoteStream
  ...
  function enableMedia(s, source, enable) {
    ...
    for (const t of tracks) {
      for (const transceiver of activeCall.connection.getTransceivers()) {
        if (
          (t.kind == CallMediaType.Audio &&
            mediaSourceFromTransceiverMid(transceiver.mid) ==
              CallMediaSource.Mic) ||
          (t.kind == CallMediaType.Video &&
            mediaSourceFromTransceiverMid(transceiver.mid) ==
              CallMediaSource.Camera)
        ) {
          if (enable) {
  ...
  function adaptToOldVersion(peerHasOldVersion, activeCall) {
    ...
    if (peerHasOldVersion) {
      ...
      if (
        activeCall.remoteStream.getVideoTracks().length > 0 ||
        activeCall.remoteTracks.get(CallMediaSource.Camera)
      ) {
        onMediaMuteUnmute("1", false);
      }
      if (
        activeCall.localMediaSources.camera &&
        !activeCall.peerMediaSources.camera
      ) {
  ...
  function webView69Or70() {
    return (
      ...
      (navigator.userAgent.includes("Chrome/69.") ||
        navigator.userAgent.includes("Chrome/70."))
    );
  }
...
function toggleRemoteVideoFitFill() {
  ...
  remote.style.objectFit =
    remote.style.objectFit != "contain" ? "contain" : "cover";
}
function toggleRemoteScreenVideoFitFill() {
  const remoteScreen = document.getElementById("remote-screen-video-stream");
  remoteScreen.style.objectFit =
    remoteScreen.style.objectFit != "contain" ? "contain" : "cover";
}
function togglePeerMedia(s, media) {
  if (!activeCall) return false;
  let res = false;
  const tracks =
    media == CallMediaType.Video ? s.getVideoTracks() : s.getAudioTracks();
  for (const t of tracks) {
    t.enabled = !t.enabled;
    res = t.enabled;
  }
...
function changeLayout(layout) {
  ...
  const localSources =
    (_a =
      activeCall === null || activeCall === void 0
        ? void 0
        : activeCall.localMediaSources) !== null && _a !== void 0
      ? _a
      : inactiveCallMediaSources;
  const peerSources =
    (_b =
      activeCall === null || activeCall === void 0
        ? void 0
        : activeCall.peerMediaSources) !== null && _b !== void 0
      ? _b
      : { mic: false, camera: false, screenAudio: false, screenVideo: false };
  if (!videos || !localSources || !peerSources) return;
  switch (layout) {
    case LayoutType.Default:
      videos.local.className = "inline";
      videos.remote.className = peerSources.screenVideo
        ? "collapsed"
        : "inline";
      videos.local.style.visibility = "visible";
      videos.remote.style.visibility = peerSources.camera
        ? "visible"
        : "hidden";
      videos.remoteScreen.style.visibility = peerSources.screenVideo
        ? "visible"
        : "hidden";
      break;
  ...
  videos.localScreen.style.visibility = localSources.screenVideo
    ? "visible"
    : "hidden";
...
function desktopShowPermissionsAlert(mediaType) {
  ...
  if (mediaType == CallMediaType.Audio) {
    window.alert(
      "Permissions denied. Please, allow access to mic to make the call working and hit unmute button. Don't reload the page.",
    );
  } else {
    window.alert(
